...
> 反射对象：就是通过反射方法，解析出来的值或者对象
>
> 接口类型变量：输出时总是以接口类型进行输出。
...
> PS:如何能获得原始值而不是副本值呢？
>
> ​	可以通过反射获得地址，通过地址获得原始值，再进行set，具体可参见【**反射的实践1】**

3.如果反射对象中包含的值不是原始值（如反射对象包含的是副本值或指向原始值的地址），则该反射对象不可以修改。
