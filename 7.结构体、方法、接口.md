...
- 使用new函数会**创建一个指向结构体类型的指针**，创建过程中会自动为结构体分配内存，结构体中每个变量被赋予对应的零值。
...
func (p Person) GetName() string {  //类似于get方法
	return p.name
}
```
...
```go
func (p *Person)SetName(name string){  //类似于set方法
	p.name = name
}
```
...
> 1. 定义接口，里面有一些方法   Method1(param_list) return_type
> 2. 一个struct实现了接口中的全部方法，即为实现了该接口（没有太多限制，谁都可以实现）
> 3. 实现了接口，才可以使用类似于多态，进行调用方法。
...
WhatType(*b) //Bird
WhatType(*d)  //Dog
WhatType(*bb)  //error    BigBird并没有实现Animal接口，故无法进行（类多态式的）参数传递
```
